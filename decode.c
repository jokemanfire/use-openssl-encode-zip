...
int main(int argn, char *argv[] ) {
    ...
    FILE * f2,* f3;
    int size = 0;
    f = fopen("test2.zip","rb+");
    size = file_size(f);
 
    f1 = fopen("test2.zip","rb+");
   
    printf("\n%d\n",size);
    fread(str,sizeof(char),size,f1);
    printf("%s\n",str);
    raw_buf = (char *)malloc(size);
    //memcpy(raw_buf,"life's a struggle",17);
    printf("------------------raw_buf\n");
    printf_buff(str,size);
    // 2
   ...
   //printf_buff(encrypt_buf,padding_size);
    ...
    decrpyt_buf(after_padding_buf,&decrypt_buf,padding_size);
    f3 = fopen("test3.zip","w+");
    printf("\n%s\n",decrypt_buf);
    fwrite(decrypt_buf,sizeof(char),padding_size,f3);
    fclose(f3);
    fclose(f1);
    //printf("------------------decrypt_buf\n");
